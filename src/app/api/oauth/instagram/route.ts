 ...
 * Redirects user to Facebook/Instagram login
 */
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(`🔐 [OAuth] Redirecting to Instagram auth for brand ${brandProfileId}`);

    return NextResponse.redirect(authUrl);
  } catch (error) {
    console.error('❌ [OAuth] Error initiating Instagram auth:', error);
    return NextResponse.json(
