...
export class MinimalPureAIContentGenerator {
  /**
   * Generate content using minimal, reliable approach
   */
  static async generateContent(request: PureAIRequest): Promise<PureAIResponse> {
    console.log('🚀 [Minimal Pure AI] Starting content generation');
    
    try {
      const prompt = this.buildMinimalPrompt(request);
      console.log('📝 [Minimal Pure AI] Generated prompt');
      ...
      console.log('✅ [Minimal Pure AI] AI response received:', content.substring(0, 200) + '...');
      ...
      console.log('✅ [Minimal Pure AI] JSON parsing successful');
      ...
      console.error('❌ [Minimal Pure AI] Generation failed:', error);
      throw error;
    }
  }
  ...
  private static validateWordCounts(content: { headline: string; subheadline: string }): { headline: string; subheadline: string } {
    ...
    if (headlineWords > 6) {
      ...
      console.log(`⚠️ [Minimal Pure AI] Headline truncated from ${headlineWords} to 6 words: "${adjustedHeadline}"`);
    }
    ...
    if (subheadlineWords > 14) {
      ...
      console.log(`⚠️ [Minimal Pure AI] Subheadline truncated from ${subheadlineWords} to 14 words: "${adjustedSubheadline}"`);
    }
  ...
  /**
   * Build a minimal, reliable prompt
   */
  private static buildMinimalPrompt(request: PureAIRequest): string {
